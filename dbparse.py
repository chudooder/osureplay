...
def parseReplayString(replayString):
    ...
    for record in records:
        ...
        time += int(w)
        if time > 0:
            action = {'time': time, 'x': float(x), 'y': float(y), 'keys': keys}
...
def parseReplay(osr):
    ...
    data['md5'], offset = parseString(osr, offset)
    data['player'], offset = parseString(osr, offset)
    data['replay_md5'], offset = parseString(osr, offset)
    data['300'], offset = parseNum(osr, offset, 2)
    data['100'], offset = parseNum(osr, offset, 2)
    data['50'], offset = parseNum(osr, offset, 2)
    data['geki'], offset = parseNum(osr, offset, 2)
    data['katu'], offset = parseNum(osr, offset, 2)
    data['miss'], offset = parseNum(osr, offset, 2)
    ...
    data['life_graph'], offset = parseString(osr, offset)
    data['time_stamp'], offset = parseNum(osr, offset, 8)

    data_len, offset = parseNum(osr, offset, 4)
    replay_str = str(lzma.decompress(osr[offset:]), 'utf-8')
...
if __name__ == "__main__":
    ...
    scoresDb = open('data/scores.db', 'rb')
